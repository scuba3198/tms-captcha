...
try {
    execSync(`npx -y esbuild "${path.join(srcDir, 'background.chrome.ts')}" --bundle --outfile="${path.join(distDir, 'background.chrome.js')}" --platform=browser --format=esm --minify`, { stdio: 'inherit' });
} catch (e) {
